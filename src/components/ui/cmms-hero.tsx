...
import { ArrowRight, Wrench, Activity, AlertTriangle, BarChart2 } from "lucide-react";
...
const workOrders = [
  { name: "Mantenimiento preventivo — Compresor A3", asset: "Planta Norte",  pct: 100, statusText: "Completado" },
  { name: "Revisión correas transportadoras",        asset: "Línea 2",       pct: 68,  statusText: "En curso" },
  { name: "Cambio de lubricante — Bomba H7",         asset: "Sala Bombas",   pct: 30,  statusText: "Pendiente" },
];

// ─── Dashboard mock ───────────────────────────────────────────────────────────
const CMMSDashboardMock = memo(() => (
  <div className="bg-white rounded-3xl shadow-[0_40px_100px_rgba(47,100,255,0.13)] border border-slate-100/80 overflow-hidden">
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          { name: "Planta Norte",   cls: "bg-blue-50 border-blue-100 text-blue-700" },
          { name: "Línea 2 — Activa", cls: "bg-emerald-50 border-emerald-100 text-emerald-700" },
        ].map((s) => (
          <div key={s.name} className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-[11px] font-semibold ${s.cls}`}>
          ...
          <div key={row.name} className="flex items-center gap-3 p-2.5 bg-slate-50/80 border border-slate-100 rounded-xl">
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1.5">
                ...
                <span className="text-[9px] text-slate-400 ml-2 whitespace-nowrap">{row.asset}</span>
              </div>
            ...
            <span
              className={`text-[10px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap border ${
                row.statusText === "Completado"
                  ? "text-emerald-700 bg-emerald-50 border-emerald-100"
                  : row.statusText === "En curso"
...
CMMSDashboardMock.displayName = "CMMSDashboardMock";

// ─── Main component ───────────────────────────────────────────────────────────
export default function CMMSHero() {
  return (
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:grid-rows-[auto_auto] gap-8 lg:gap-x-16 xl:gap-x-24 lg:gap-y-10 lg:items-center">
          ...
          <motion.div {...fadeUpSub} className="lg:col-start-1 lg:row-start-2 lg:self-start">
            <p className="text-lg text-slate-600 font-light leading-relaxed mb-10 max-w-[30rem]">
              Implementamos sistemas CMMS a medida que digitalizan las órdenes de trabajo, programan mantenimientos preventivos, gestionan inventario de repuestos e integran sensores IoT para anticipar fallos antes de que ocurran.
            </p>
