...
// ─── Static config — module-level, never recreated ────────────────────────────
...
// ─── Shared CTA handler — avoids duplicate inline logic ───────────────────────
function handleServiceCta(href: string, e: React.MouseEvent): void {
  if (href.startsWith("#")) {
    e.preventDefault();
...
// ─── Feature Bullet ───────────────────────────────────────────────────────────
const FeatureBullet = ({ text, index }: { text: string; index: number }) => (
  <motion.li
...
const ContentPanel = ({ service }: { service: ServiceItem }) => {
  const { href } = SERVICE_CONFIG[service.key];
  return (
    <motion.div
      key={service.key}
...
const DesktopSplitPanel = ({ services }: { services: ServiceItem[] }) => {
  ...
  return (
    <div className="grid grid-cols-[280px_1fr] rounded-3xl border border-slate-200 overflow-hidden shadow-[0_8px_48px_rgba(7,21,64,0.07)] bg-white min-h-[520px]">
      ...
      <div className="bg-white relative overflow-hidden">
        <div
          ...
          style={{
            backgroundImage: "linear-gradient(#071540 1px, transparent 1px), linear-gradient(90deg, #071540 1px, transparent 1px)",
            backgroundSize: "32px 32px",
          }}
        />
        <div className="relative z-10 h-full">
          <AnimatePresence mode="wait">
            {activeService && <ContentPanel key={activeService.key} service={activeService} />}
          </AnimatePresence>
        </div>
      </div>
    </div>
...
const MobileAccordion = ({ services }: { services: ServiceItem[] }) => {
  ...
  const toggle = useCallback((key: string) => setOpenKey((prev) => (prev === key ? "" : key)), []);
              ...
              {isOpen && (
                ...
                >
                  <div className="px-5 pb-5 border-t border-slate-100">
                    ...
                    <p className="text-slate-500 font-light text-sm leading-relaxed mb-5">{svc.description}</p>
...
export default function ServicesSplitPanel() {
  ...
  // Memoized: only rebuilt when translations change (never in practice)
  ...
  return (
    <section id="servicios" className="py-20 lg:py-28 bg-[#F8FAFC] relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        ...
        >
          {/* Conditional rendering — only the active breakpoint mounts */}
          {breakpoint === "desktop" && <DesktopSplitPanel services={services} />}
          {breakpoint === "tablet"  && <TabletTabs        services={services} />}
          {breakpoint === "mobile"  && <MobileAccordion   services={services} />}
        </motion.div>
