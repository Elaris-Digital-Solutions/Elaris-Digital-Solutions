...
import { ArrowRight, GitMerge, ShieldCheck, Zap, Activity } from "lucide-react";
...
const endpoints = [
  { method: "GET",    path: "/api/v1/clientes",         status: "200", active: true },
  { method: "POST",   path: "/api/v1/pedidos",           status: "201", active: true },
  { method: "PATCH",  path: "/api/v1/inventario/{id}",   status: "200", active: true },
];
  ...
  <div className="bg-white rounded-3xl shadow-[0_40px_100px_rgba(47,100,255,0.13)] border border-slate-100/80 overflow-hidden">
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          { name: "OAuth 2.0",    cls: "bg-blue-50 border-blue-100 text-blue-700" },
          { name: "JWT Bearer",   cls: "bg-blue-50 border-blue-100 text-blue-700" },
        ].map((m) => (
      ...
      <div className="space-y-2">
        {endpoints.map((ep) => (
          <div
            key={ep.path}
            className="flex items-center gap-3 p-2.5 bg-slate-50/80 border border-slate-100 rounded-xl"
          >
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${methodColors[ep.method]}`}>
              {ep.method}
            </span>
            <div className="flex-1 min-w-0">
              <span className="text-xs font-mono text-[#071540] truncate block">{ep.path}</span>
            </div>
...
APIGatewayMock.displayName = "APIGatewayMock";

// ─── Main component ───────────────────────────────────────────────────────────
export default function ApiIntegrationHero() {
  return (
